...
""";
        ...
        /// <summary>Layout-extension keys under which the spec travels with the report.</summary>
        public const string SpecExtensionKey = "XafAIReportDesigner.Spec";
        public const string PromptExtensionKey = "XafAIReportDesigner.Prompt";

        public static string BuildSystemPrompt(string schemaText) =>
            SpecInstructions + "\n\nDATABASE SCHEMA:\n" + schemaText;

        /// <summary>System prompt for spec-level modification: same contract, seeded with the current spec.</summary>
        public static string BuildModifySystemPrompt(string schemaText, string currentSpecJson) =>
            SpecInstructions +
            "\n\nCURRENT SPEC (the report as it exists now):\n" + currentSpecJson +
            "\n\nApply the user's requested change to the CURRENT SPEC. Keep everything not mentioned unchanged. " +
            "Output ONLY the complete updated JSON object." +
            "\n\nDATABASE SCHEMA:\n" + schemaText;

        /// <summary>Embeds the spec (and originating prompt) in the report layout so it survives save/load.</summary>
        public static void AttachSpec(XtraReport report, string specJson, string? prompt)
        {
            report.Extensions[SpecExtensionKey] = specJson;
            if (!string.IsNullOrEmpty(prompt)) report.Extensions[PromptExtensionKey] = prompt;
        }

        /// <summary>Returns the embedded spec JSON, or null for reports not generated by this pipeline.</summary>
        ...
        private static XRTable BuildRow(List<ColumnSpec> columns, float pageWidth, bool isHeader)
        {
            ...
            table.Rows.Add(row);
            // The first TEXT column (not right-aligned) gets double weight — descriptions
            // need the room regardless of where the user ordered them.
            int wideIndex = Math.Max(0, columns.FindIndex(c => !c.RightAlign));
            ...
            for (int i = 0; i < columns.Count; i++)
            {
                ...
                {
                    WidthF = i == wideIndex ? unit * 2 : unit,
